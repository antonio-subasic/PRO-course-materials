...
public class WordGuess
{
    ...
    public virtual string GetRandomWord() => AvailableWords[Random.Shared.Next(AvailableWords.Length)];
    ...
    /// </remarks>
    public virtual string GetInitialGuess() => new Regex("[a-zA-Z]").Replace(WordToGuess, "_");
    ...
    {
        var currentGuess = CurrentGuess.ToCharArray();
        var found = false;
        ...
        CurrentGuess = new string(currentGuess);
        return found;
    }
}
...
public class EasyWordGuess : WordGuess
{
    ...
    {
        var initialGuess = base.GetInitialGuess().ToCharArray();

        if (WordToGuess.Length >= 4)
        {
            var characters = new Dictionary<char, HashSet<int>>();

            for (var i = 0; i < WordToGuess.Length; i++)
            {
                if (char.IsLetter(WordToGuess[i]))
                {
                    var c = char.ToLower(WordToGuess[i]);

                    if (!characters.TryGetValue(c, out var value)) { characters.Add(c, [i]); }
                    else { value.Add(i); }
                }
            }
            ...
            for (var i = 0; i < 3; i++)
            {
                var index = Random.Shared.Next(characters.Count);
                foreach (var j in characters.ElementAt(index).Value) { initialGuess[j] = WordToGuess[j]; }
                characters.Remove(characters.ElementAt(index).Key);
            }
        }

        return new string(initialGuess);
    }
}
...
public class HardWordGuess : WordGuess
{
    ...
    public override bool Guess(char letter)
    {
        ...
        for (var i = 0; i < WordToGuess.Length; i++)
        {
            if (WordToGuess[i] == letter && currentGuess[i] == '_')
            {
                currentGuess[i] = WordToGuess[i];
                CurrentGuess = new string(currentGuess);
                return true;
